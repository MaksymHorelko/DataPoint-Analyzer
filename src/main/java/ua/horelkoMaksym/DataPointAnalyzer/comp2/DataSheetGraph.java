package ua.horelkoMaksym.DataPointAnalyzer.comp2;
...
public class DataSheetGraph extends JPanel implements Serializable {
	...
	private double minX() {
		...
		if (dataSheet != null) {
			...
			for (int i = 0; i < size; i++)
				if (dataSheet.getDataPoint(i).getX() < result)
					result = dataSheet.getDataPoint(i).getX();
		}
		return result;
	}
	...
	private double maxY() {
		...
		if (dataSheet != null) {
			int size = dataSheet.size();
			for (int i = 0; i < size; i++) {
				if (dataSheet.getDataPoint(i).getY() > result) {
					result = dataSheet.getDataPoint(i).getY();
				}
			}
		}
		return result;
	...
	public void showGraph(Graphics2D gr) {
		...
		xMin = minX() - deltaX; // Застосовуємо зміщення
		xMax = maxX() + deltaX; // Застосовуємо зміщення
		yMin = minY() - deltaY; // Застосовуємо зміщення
		yMax = maxY() + deltaY; // Застосовуємо зміщення
		...
		if (dataSheet != null) {
			if (!isConnected) {
				for (int i = 0; i < dataSheet.size(); i++) {
					double x = x0 + (dataSheet.getDataPoint(i).getX() * xScale);
					double y = y0 - (dataSheet.getDataPoint(i).getY() * yScale);
				...
				double xOld = x0 + dataSheet.getDataPoint(0).getX() * xScale;
				double yOld = y0 - dataSheet.getDataPoint(0).getY() * yScale;
